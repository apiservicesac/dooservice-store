...
for docker_compose_file in $docker_compose_files; do
  ...
  if [[ "$image" == *":"* ]]; then
    ...
    if [[ "$current_config_version" != "$trimmed_version" ]]; then
      ...
      # ------------------- Update config.json -------------------
      dooservice_version=$(jq -r '.dooservice_version' "$config_file")
      dooservice_version=$((dooservice_version + 1))

      created_at=$(jq -r '.created_at // 0' "$config_file")
      updated_at=$(date +%s | awk '{print int($1*1000)}')

      contents="$(jq --argjson dooservice_version "$dooservice_version" --argjson created_at "$created_at" --argjson updated_at "$updated_at" '.dooservice_version = $dooservice_version | .created_at = $created_at | .updated_at = $updated_at' "$config_file")"
      echo "${contents}" >"$config_file"
